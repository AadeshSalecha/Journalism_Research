...
def main():
	...
	print('%-40s %-10s %-10s %-10s %-10s %-5s %-5s' % ("News Org", "Input", "RTs", "NoLinks", "NoNewsLink", "Broke", "Twitter"), file=stats)
	for f in files:
		raw_data = []
		data = []
		...
		with open(mypath + "output/Result" + f, mode='w', encoding="utf-8") as outptr:
			...
			for i in range(10): #len(data)):
				...
				for link in links:
					...
					# if page doesn't exists or is a link to a tweet
					if(page.ok == False):
						broken += 1
						break;

					if(page.url.find("twitter.com") != -1):
						# page = requests.get(getTwittertoNewsLink(page.url))
						# print(page.url)
						twitter += 1
						break
					...
					article_index += 1				
					if(len(images) == 0):
						writer.writerow([article_index, f[:f.find(".out")], page.url, date, len(images),"NA", "NA"])

					for img_num in range(min(CONST_MAX_IMG, len(images))):
						if(img_num == 0):
							writer.writerow([article_index, f[:f.find(".out")], page.url, date, len(images), "NA" if(images[img_num][0] == "") else images[img_num][0], "NA" if(images[img_num][1] == "") else images[img_num][1]])
						else:
							writer.writerow([article_index, f[:f.find(".out")], "", "", len(images), "NA" if(images[img_num][0] == "") else images[img_num][0], "NA" if(images[img_num][1] == "") else images[img_num][1]])

		print(' %-10s %-5s %5s' % (len(data)-article_index, broken, twitter), file=stats)
	stats.close()
