    ...
    for f in cur_files:
      ...
      for follower in all_followers:
        ...
        if(not(follower in all_done)):
          seen += generateFollowers(follower, cur_level)
          all_done[follower] = True

        print("Total nodes processed = ", len(all_done))
        print("Total nodes seen = ", seen)

  convertToNewFormat()

def generateFollowers(org, level):
  # Open page
  ...
  with open("result_output/Level" + str(level) + "/followers_" + org, mode='w', encoding="utf-8") as outptr:
