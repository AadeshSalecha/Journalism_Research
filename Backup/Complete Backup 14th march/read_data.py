...
if __name__ == '__main__':
	if(len(sys.argv) > 2 and sys.argv[2] == "test"):
		# link = "https://www.ajc.com/weather/sunday-weather-traffic-cloud-cover-keep-georgia-cold-rain-looms/XqPhBXf4z9rfwrKgiDCP8N/?mode=new"
		# page = requests.get(link)
		# soup = BeautifulSoup(page.text, 'html.parser')
		...
		with open(mypath + "input/" + f, mode='r') as inptr:
			...
			for row in reader:		
				...
				cc = row[5]
				if(cr == "NA" and cc != "NA"):
					cr = cc[cc.rfind('['):] if(cc.rfind('[') != -1) else cc[cc.rfind('('):] if(cc.rfind('(') != -1) else cc[cc.rfind('Photo'):] if(cc.rfind('Photo') != -1) else "NA"

				raw_data.append([row[0], row[1], row[2], row[3], row[4], row[5], cr])
